...
FEEDBACK_CATEGORIES = {
    ...
    "소재·착용감": [
        "따뜻", "부드럽", "도톰", "두껍", "얇", "촉감", "소재", "원단",
        "탄탄", "포근", "폭신", "쿠션", "착화감", "발볼", "발폭", "밑창",
        "인솔", "땀", "통기", "신축", "늘어나", "줄어", "무게", "가볍",
        "무겁", "편안", "편하", "불편",
    ],
    "사이즈·핏": [
        "사이즈", "기장", "허리", "밴드", "핏", "오버핏", "슬림핏",
        "품", "폭", "길이", "실측",
        "크다", "크네요", "커요", "작다", "작네요", "작아요",
        "타이트", "슬림", "루즈", "넉넉", "낑기", "끼다",
        "짧다", "짧아요", "짧네요", "길다", "길어요",
    ],
    "디자인·컬러": [
...
NEGATIVE_KEYWORDS = [
    ...
    "최악이에요", "최악이다", "최악",
    # "불편한" 제거 → "없었어요" 등 부정문에서 false positive 발생
    "불편해요", "불편합니다", "불편하다", "불편함이", "불만이에요", "불만족",
    # 품질 문제
    "터졌어요", "터졌", "뜯어짐", "뜯겼", "불량이에요", "불량품", "불량",
    "하자가", "하자 있", "오염됐", "변형됐",
    "줄어들었", "줄어들어요", "줄었어요", "수축됐",
    "올풀림", "올이 나", "오염이 있", "오염 있어", "얼룩이 있",
    ...
    "사진이랑 달라", "사진과 달라",
    "색이 다르", "색상이 다르", "실물이랑 달라",
    # 가격/후회
    "아깝다", "아까워요", "아까워서", "돈 아까워", "돈아까워",
    "후회해요", "후회됩니다", "후회했",
    "가격이 아까워", "비싼 것 같아",
    # 반품/교환 의도
    "환불했", "환불 요청", "반품했", "반품 요청",
...
POSITIVE_KEYWORDS = [
    ...
    "예쁘다", "예뻐요", "예쁩니다", "예쁘게", "예쁜",
    "이쁘다", "이뻐요", "이쁘게", "이쁩니다", "이쁜",
    "딱이다", "딱이에요", "딱이네요", "딱이야", "딱 맞아요",
    "재구매", "또 살", "또 샀", "또 구매",
    "추천해요", "추천합니다", "추천드립니다", "추천드려요", "강추", "강력 추천",
    "완벽해요", "완벽합니다",
    "빠르다", "빠르게", "빨리 왔",
    "친절해요", "친절하게",
    "기대 이상", "기대보다 좋", "최고예요", "최고입니다",
    "마음에 들어요", "마음에 들어", "맘에 들어", "맘에들어", "마음에들어",
    # 착용감·소재 긍정
    "편해요", "편합니다", "편하고", "편안해요", "편안합니다", "편안하고",
    "따뜻해요", "따뜻합니다", "따뜻하고", "따뜻한",
    "부드러워요", "부드럽고", "부드러운",
    # 전반 긍정
    "괜찮아요", "괜찮습니다",
...
def process(input_path: str, output_path: str) -> None:
    ...
    # (1) 버전을 우선 탐색, 없으면 기본 파일명 시도
    import os
    base_dir = os.path.dirname(os.path.abspath(input_path))
    supplement_path = None
    for candidate in ("미쏘, 슈펜(1).csv", "미쏘, 슈펜.csv"):
